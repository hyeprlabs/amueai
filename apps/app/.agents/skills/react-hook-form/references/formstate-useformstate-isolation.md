...
**Scope it further with `name`.** An isolated `useFormState({ control })` still re-renders on _any_ field's state change. Pass `name` to narrow it to the fields the component actually displays:
